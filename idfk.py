...
class TimeDelta(datetime.timedelta):
    def __new__(cls, timedelta):
        self = super().__new__(cls, timedelta.days, timedelta.seconds, timedelta.microseconds)
        self.minutes = self.seconds // 60
        self.hours = self.minutes // 60
        return self

    def display(self):
        if not self.days and not self.hours and not self.minutes:
            return 'Less than a minute ago'
        elif self.days:
            return f'{self.days} day{"s" if self.days > 1 else ""} ago'
        elif self.hours:
            return f'{self.hours} day{"s" if self.hours > 1 else ""} ago'
        else:
